...
@Command(name = "compare", headerHeading = "%n", header = "Compares Avro Schemas generated from Tables in a Database", synopsisHeading = "%n", descriptionHeading = "%nDescription:%n%n", description = """
        Given 2 directories (CURRENT / NEXT) containing Avro Schemas of Database Tables,
        compares them according to Compatibility Level.""", parameterListHeading = "%nParameters:%n", optionListHeading = "%nOptions:%n")
public class CompareCommand extends BaseComparisonCommand {
    ...
    @Parameters(arity = "1", index = "0", paramLabel = "CURR_SCHEMAS_DIR", description = "Directory with the CURRENT Database Table schemas")
    Path currSchemasDir;

    @Parameters(arity = "1", index = "1", paramLabel = "NEXT_SCHEMAS_DIR", description = "Directory with the NEXT Database Table schemas")
    Path nextSchemasDir;
    ...
    public Integer call() {
        ...
        try {
            ...
            // NEW: Check for schema changes without table changes in CI mode
    ...
    private void validate() throws CommandLine.ParameterException {
        ...
        if (!currSchemasDirFile.exists() || !currSchemasDirFile.isDirectory()) {
            throw new CommandLine.ParameterException(spec.commandLine(), "Directory must exist: " + currSchemasDirFile.getAbsolutePath());
        }

        final File nextSchemasDirFile = nextSchemasDir.toFile();
        if (!nextSchemasDirFile.exists() || !nextSchemasDirFile.isDirectory()) {
            throw new CommandLine.ParameterException(spec.commandLine(), "Directory must exist: " + nextSchemasDirFile.getAbsolutePath());
        }
    ...
    /// It's left to the calling logic to decide when to fail/succeed the actual CLI
    /// command.
    ///
    /// @param currentSchemasDir          [Path] to the directory containing the CURRENT
    ///                                                                                                                                                                           Table Schemas compared.
    /// @param nextSchemasDir             [Path] to the directory containing the NEXT Table
    ///                                                                                                                                                                           Schemas compared.
    /// @param compatibilityLevel         [CompatibilityLevel] used during the comparison.
    /// @param keyIncompatibilities:      [Map] of Table Avro Schemas identifiers, to
    ///                                                                                                                                                                           [List] of Key Schema incompatibilities.
    /// @param valueIncompatibilities:    [Map] of Table Avro Schemas identifiers, to
    ///                                                                                                                                                                           [List] of Value Schema incompatibilities.
    /// @param envelopeIncompatibilities: [Map] of Table Avro Schemas identifiers,
    ///                                                                                                                                                                           to [List] of Envelope Schema incompatibilities.
    /// @param removedTables:             [Set] of Table Avro Schemas that were removed: present
    ///                                                                                                                                                                           in CURRENT but absent from NEXT.
    /// @param addedTables:               [Set] of Table Avro Schemas that were added: absent from
    ///                                                                                                                                                                           CURRENT but present in NEXT.
    record Result(@JsonProperty(required = true, index = 0) @Nonnull Path currentSchemasDir,
                  @JsonProperty(required = true, index = 1) @Nonnull Path nextSchemasDir,
                  @JsonProperty(required = true, index = 2) @Nonnull CompatibilityLevel compatibilityLevel,
                  @JsonProperty(required = true, index = 3) @Nonnull Map<String, List<String>> keyIncompatibilities,
                  @JsonProperty(required = true, index = 5) @Nonnull Map<String, List<String>> valueIncompatibilities,
                  @JsonProperty(required = true, index = 7) @Nonnull Map<String, List<String>> envelopeIncompatibilities,
                  @JsonProperty(required = true, index = 10) @Nonnull Set<String> removedTables,
                  @JsonProperty(required = true, index = 11) @Nonnull Set<String> addedTables,
                  @JsonProperty(required = true, index = 12) @Nonnull Map<String, Boolean> keySchemaChanged,
                  @JsonProperty(required = true, index = 13) @Nonnull Map<String, Boolean> valueSchemaChanged,
                  @JsonProperty(required = true, index = 14) @Nonnull Map<String, Boolean> envelopeSchemaChanged) {
        ...
        public Set<String> tablesWithChanges() {
            ...
            keySchemaChanged.entrySet().stream().filter(Map.Entry::getValue).forEach(entry -> tablesWithChanges.add(entry.getKey()));

            valueSchemaChanged.entrySet().stream().filter(Map.Entry::getValue).forEach(entry -> tablesWithChanges.add(entry.getKey()));

            envelopeSchemaChanged.entrySet().stream().filter(Map.Entry::getValue).forEach(entry -> tablesWithChanges.add(entry.getKey()));
        ...
        public boolean hasAnySchemaChanges() {
            return keySchemaChanged.values().stream().anyMatch(Boolean::booleanValue) || valueSchemaChanged.values().stream().anyMatch(Boolean::booleanValue) || envelopeSchemaChanged.values().stream().anyMatch(Boolean::booleanValue);
        }
        ...
        public boolean hasSchemaChangesWithoutTableChanges() {
            return hasAnySchemaChanges() && removedTables.isEmpty() && addedTables.isEmpty();
        }
        ...
        public static Result build(@Nonnull Path currSchemasDir, @Nonnull Path nextSchemasDir, @Nonnull CompatibilityLevel compatibilityLevel) throws IOException {
            ...
            // NEW: Initialize change tracking maps
