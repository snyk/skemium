...
@Command(name = "compare-files", headerHeading = "%n", header = "Compares two Avro Schema (.avsc) files", synopsisHeading = "%n", descriptionHeading = "%nDescription:%n%n", description = """
        Given 2 Avro Schema files (.avsc), compares them according to the specified Compatibility Level.
        This command is designed for comparing bespoke/custom Avro schemas that are not generated from database tables.""", parameterListHeading = "%nParameters:%n", optionListHeading = "%nOptions:%n")
public class CompareFilesCommand extends BaseComparisonCommand {
    ...
    @Parameters(arity = "1", index = "0", paramLabel = "CURR_SCHEMA_FILE", description = "Path to the CURRENT Avro schema file (.avsc)")
    Path currentSchemaFile;

    @Parameters(arity = "1", index = "1", paramLabel = "NEXT_SCHEMA_FILE", description = "Path to the NEXT Avro schema file (.avsc)")
    Path nextSchemaFile;
