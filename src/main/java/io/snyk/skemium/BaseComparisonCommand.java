...
public abstract class BaseComparisonCommand extends BaseCommand {
    ...
    @Option(names = { "-c",
            "--compatibility" }, defaultValue = "${env:COMPATIBILITY}", showDefaultValue = CommandLine.Help.Visibility.ALWAYS, description = """
    ...
    @Option(names = { "-i", "--ci",
            "--ci-mode" }, defaultValue = "${env:CI_MODE}", showDefaultValue = CommandLine.Help.Visibility.ALWAYS, description = """
                    CI mode - Fail when schema changes are detected (env: CI_MODE - optional)""")
    protected Boolean ciMode = false;

    @Option(names = { "-o",
            "--output" }, defaultValue = "${env:OUTPUT_FILE}", description = "Output file (JSON); overridden if exists (env: OUTPUT_FILE - optional)")
